...
随机朴素卷积对拍。2026-08-31 新增的两份动态开点线段树与 2026-09-01
新增的线性递推，以及同日更新的 KMP、Z 函数与 exKMP 尚未重新运行语法
验证；Z 函数的官方结果仍来自更新前的实现。
...
9. 2026-08-31 内容更新：在线段树与懒标记线段树二级主题下分别新增动态
   开点实现，普通实现与动态开点实现分别使用三级标题。
10. 2026-09-01 内容更新：数学章新增 Kitamasa 型线性递推模板，在
    $O(k^2 \log n)$ 时间内求齐次或常数非齐次递推第 $n$ 项，并支持同时
    求前缀和。
11. 2026-09-01 字符串更新：KMP 模板拆出 `PrefixFunction` 并增加完整
    匹配接口 `KMP`；扩展 KMP 的公开接口统一为 `ExKMP`。
12. 2026-09-04 图论更新：新增边权仅为 `0/1` 的 `BFS01` 单源最短路
    模板，并补充 BFS、0-1 BFS 与 Dijkstra 共用的多源初始化用法；
    Dijkstra、0-1 BFS 与多源初始化归入同一“最短路”二级主题。
    Library Checker 暂无对应的 0-1 BFS 专用题目，catalog 中明示为未独立验证。
13. 2026-09-04 排版更新：全量统一正文数学语义，复杂度、区间、约束和
    等式改用公式排版，反引号仅保留给 C++ 接口与可复制代码；
    PDF 正文字体固定为 LXGW WenKai，代码仍使用原有等宽字体；
    删除已被 `book.typ + src/**/*.typ` 取代且无任何引用的旧单文件副本
    `mtf.typ`。
14. 2026-09-05 封板更新：图论章新增最小生成树主题，同时收录 Kruskal
    与堆优化 Prim；树上问题新增虚树与 DSU on Tree，LCA 补充
    `IsAncestor` 接口。四份新模板已进入 catalog，尚未运行语法与正确性验证。

## 最终产物与实测

| 产物 | 说明 | 实测 |
| --- | --- | --- |
| `preview/index.html` | 单文件离线模板站 | 当前 catalog 为 55 卡片、19 徽章；本轮未运行产物质量门禁 |
...
- 36 个模板尚无官方数据验证；KMP、浮点 FFT 与组合数等需要本地对拍或
  适配验证契约。浮点 FFT 当前有随机对拍结果，但没有官方验证徽章；
  新增的两份动态开点线段树、线性递推、MST、虚树、DSU on Tree 与更新后
  的字符串模板尚未运行语法门禁，Z 函数需重跑官方数据以刷新结果；
