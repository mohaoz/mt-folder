...
**37 个模板、15 个有官方数据验证（14 项检查全部 AC）、其余 21 个
未覆盖模板全部通过语法编译门禁**。
...
3. 内容扩充：新增可并堆（pb_ds）、矩阵快速幂、高维前缀和、
   势能线段树、可持久化权值线段树、Kruskal 重构树六个模板与
   线段树章节的 Kadane 实例；Dinic 换扁平存边并按层截断 BFS，
...
| 产物 | 说明 | 实测 |
| --- | --- | --- |
| `preview/index.html` | 单文件离线模板站 | 37 卡片、15 徽章、零外部依赖、零重复 ID |
...
| `yosupo/` | 生成的提交与 manifest | 14/14 AC，语法门禁 21/21 |

验证结果（本地全量）：

- 14 项官方检查全部 AC，含最慢用例耗时列与 TLE 复核留痕机制；
- 单元测试 51 个全部通过（含产物质量门禁）；
- 未覆盖模板语法编译 21/21 通过。

2026-07-28 从仓库目录使用项目内缓存重新全量验证：14/14 AC、
21/21 语法门禁通过；Dinic 最慢用例为 3.4s/5s，触发 60% 临界警告但
...
- 22 个模板尚无官方数据验证；其中 KMP、FFT、组合数等在 Library
